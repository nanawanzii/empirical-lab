...
const i18n = {
  ...
  "hero_desc": { en: "Built for economics master's students who already know basic R/Python and want to master causal inference, econometrics packages, data acquisition, and reproducible research workflows.", zh: "面向已掌握基础 R/Python 的经济学/社会学/政治学学生，训练因果推断、计量包、数据获取和可复现研究流程。" },
  ...
  "prereq_iv": { en: "Complete DID & Event Study Boss", zh: "完成 DID 与事件研究 Boss" },
  "prereq_rdd": { en: "Complete IV/2SLS Boss", zh: "完成 IV/2SLS Boss" },
  "prereq_matching": { en: "Complete DID & Event Study Boss", zh: "完成 DID 与事件研究 Boss" },
  "prereq_ts": { en: "Complete Fixed Effects Lab Boss", zh: "完成固定效应实验室 Boss" },
  ...
  "prereq_iv_full": { en: "Complete the DID & Event Study boss.", zh: "完成 DID 与事件研究 Boss" },
  "prereq_rdd_full": { en: "Complete the IV/2SLS boss.", zh: "完成 IV/2SLS Boss" },
  "prereq_matching_full": { en: "Complete the DID & Event Study boss.", zh: "完成 DID 与事件研究 Boss" },
  "prereq_ts_full": { en: "Complete the Fixed Effects Lab boss.", zh: "完成固定效应实验室 Boss" },
  // Quest outcomes
...
const badges = [
  ...
  ["iv-master", "Instrument Wielder", "Complete the IV/2SLS boss."],
  ["rdd-master", "Discontinuity Detective", "Complete the RDD boss."],
  ["matching-master", "Counterfactual Constructor", "Complete the Matching/DML boss."],
  ["lp-master", "Impulse Tracer", "Complete the Time Series/LP boss."]
];
...
const quests = [
  ...
  { id: "iv", title: "Instrumental Variables", reward: "Instrument Wielder", summary: "Endogeneity, exclusion restriction, 2SLS mechanics, weak instruments, LATE, judge-leniency IV, and Bartik/shift-share designs.", tags: ["IV/2SLS", "Causal Inference", "LATE"], levels: ["iv-1", "iv-2", "iv-3", "iv-4", "iv-5", "iv-6", "iv-7", "iv-boss"] },
  { id: "rdd", title: "Regression Discontinuity", reward: "Discontinuity Detective", summary: "Running variables, bandwidth selection, local polynomial estimation, manipulation tests, fuzzy RD, and visualization best practices.", tags: ["RDD", "Causal Inference", "Local Randomization"], levels: ["rdd-1", "rdd-2", "rdd-3", "rdd-4", "rdd-5", "rdd-6", "rdd-boss"] },
  { id: "matching", title: "Matching & Double ML", reward: "Counterfactual Constructor", summary: "Selection on observables, propensity scores, matching estimators, IPW, doubly robust estimation, and debiased machine learning.", tags: ["Matching", "PSM", "IPW", "AIPW", "Double ML"], levels: ["match-1", "match-2", "match-3", "match-4", "match-5", "match-6", "match-boss"] },
  { id: "ts", title: "Time Series & Local Projections", reward: "Impulse Tracer", summary: "Stationarity, VARs, local projections, LP vs VAR tradeoffs, LP-IV, and LP-DiD for causal impulse responses.", tags: ["Time Series", "Local Projections", "VAR", "LP-IV"], levels: ["ts-1", "ts-2", "ts-3", "ts-4", "ts-5", "ts-6", "ts-boss"] }
];
...
const levels = {
  ...
  "iv-1": { quest: "iv", n: 1, title: "The Endogeneity Problem", xp: 30, skills: ["iv"], type: "mcq", story: "You estimate the effect of police spending on crime rates using OLS. A referee writes: \"This coefficient is uninterpretable.\"", concept: "OLS fails when the regressor is correlated with the error term. Common sources: omitted variables (cities that spend more on police may also differ on poverty), reverse causality (high crime → more police spending), and measurement error. Endogeneity biases the OLS coefficient in an unknown direction.", question: "Why might OLS produce a biased estimate of the effect of police on crime?", options: ["Omitted variables and reverse causality correlate the regressor with the error", "The sample size is always too small", "Fixed effects cannot be computed", "Python does not support regression"], answer: 0, feedback: "Correct. When X is correlated with ε, the OLS coefficient does not identify a causal effect." },
  "iv-2": { quest: "iv", n: 2, title: "IV Logic and Assumptions", xp: 30, skills: ["iv"], type: "mcq", story: "A colleague proposes using \"distance to the nearest military base\" as an instrument for police spending. You need to evaluate whether this is a valid IV.", concept: "A valid instrument Z must satisfy: (1) Relevance — Z predicts the endogenous X (first stage); (2) Exclusion restriction — Z affects Y only through X; (3) Independence/Exogeneity — Z is uncorrelated with unobserved confounders. The exclusion restriction is untestable but must be argued on institutional grounds.", question: "Which assumption requires that the instrument affects the outcome ONLY through the endogenous regressor?", options: ["Exclusion restriction", "Relevance condition", "Homoskedasticity", "Stationarity"], answer: 0, feedback: "Correct. The exclusion restriction says Z → X → Y with no direct Z → Y path." },
  "iv-3": { quest: "iv", n: 3, title: "2SLS Mechanics", xp: 35, skills: ["iv"], type: "code", story: "Time to implement 2SLS. The first stage regresses the endogenous variable on the instrument(s) and exogenous controls; the second stage uses fitted values.", concept: "2SLS proceeds in two stages: (1) First stage: X̂ = π₀ + π₁Z + γW + v; (2) Second stage: Y = β₀ + β₁X̂ + δW + ε. The reduced form (Y on Z directly) divided by the first stage gives the IV estimate: β₁ = Cov(Y,Z)/Cov(X,Z). In fixest: feols(Y ~ exog | FE | endog ~ instrument).", question: "Complete the fixest IV specification: regress wages on education (endogenous) instrumented by quarter_of_birth, controlling for age with no fixed effects.", starter: "feols(wages ~ age | 0 | _____ ~ _____, data = df)", required: ["education", "quarter_of_birth"], pattern: "education\\s*~\\s*quarter_of_birth", hint: "The endogenous variable goes on the left of ~, the instrument on the right.", feedback: "Good. fixest uses the endog ~ instrument syntax after the second pipe." },
  "iv-4": { quest: "iv", n: 4, title: "First-Stage Diagnostics", xp: 35, skills: ["iv"], type: "mcq", story: "Your first-stage regression shows an F-statistic of 4.2 on the excluded instrument. The standard errors in the second stage look suspiciously large.", concept: "A weak instrument (first-stage F < 10, or more conservatively the Stock-Yogo critical values) means the instrument barely predicts X. Consequences: 2SLS is biased toward OLS, confidence intervals have poor coverage, and the estimator may be unreliable even in large samples. The effective F-stat (Olea & Pflueger 2013) or the tF procedure (Lee et al. 2022) are modern diagnostics.", question: "What does a first-stage F-statistic well below 10 indicate?", options: ["The instrument is weak — IV estimates may be severely biased and unreliable", "The instrument is too strong", "The second stage is unnecessary", "Clustering should be removed"], answer: 0, feedback: "Correct. Weak instruments cause finite-sample bias, size distortion, and poor inference." },
  "iv-5": { quest: "iv", n: 5, title: "LATE and Complier Interpretation", xp: 35, skills: ["iv"], type: "mcq", story: "Your IV estimates the effect of attending college on earnings, using draft lottery eligibility as the instrument. A reviewer asks: \"For whom does this estimate hold?\"", concept: "Under heterogeneous treatment effects and a binary instrument, IV identifies the Local Average Treatment Effect (LATE) — the effect for compliers (those whose treatment status is changed by the instrument). Always-takers (attend regardless of instrument) and never-takers (never attend regardless) are not in the estimand. Monotonicity assumes no defiers.", question: "The IV estimate from a draft-lottery instrument identifies the effect for which subpopulation?", options: ["Compliers — those whose college attendance is changed by draft eligibility", "The entire population equally", "Always-takers who attend college regardless", "Never-takers who never attend college"], answer: 0, feedback: "Correct. LATE applies to compliers, not the full sample. External validity requires understanding who compliers are." },
  "iv-6": { quest: "iv", n: 6, title: "Judge/Examiner IV Design", xp: 40, skills: ["iv"], type: "mcq", story: "Collinson et al. (QJE 2024) study the causal effect of eviction on tenant outcomes using quasi-random assignment of housing court judges who vary in leniency.", concept: "The judge-leniency (or examiner) design uses quasi-random assignment of cases to decision-makers (judges, examiners, loan officers) as an instrument. The instrument is typically the leave-out mean of the judge's decision rate across other cases. Key requirements: (1) conditional random assignment; (2) monotonicity (a stricter judge is stricter for everyone); (3) exclusion (judge affects outcomes only via the decision). Baron et al. (QJE 2024) formalize inference and testable implications of the examiner design.", question: "In the judge-leniency IV, what serves as the instrument for the endogenous treatment (e.g., eviction)?", options: ["The judge's leave-out leniency rate — their tendency to evict in other cases", "The tenant's income level", "The number of court rooms", "The calendar year of filing"], answer: 0, feedback: "Correct. The leave-out mean of a quasi-randomly assigned judge's decisions is the instrument, per Collinson et al. (QJE 2024)." },
  "iv-7": { quest: "iv", n: 7, title: "Shift-Share / Bartik Instruments", xp: 40, skills: ["iv"], type: "ordering", story: "You want to estimate how immigration affects local wages. Following Goldsmith-Pinkham et al. (AER 2020) and Borusyak et al. (REStud 2022), you construct a Bartik instrument combining initial settlement shares with national immigration shocks.", concept: "A shift-share (Bartik) instrument B_it = Σ_k s_{ik,0} × g_{kt} combines pre-period exposure shares (s) with aggregate shocks (g). Identification can come from exogenous shares (GPSS 2020: shares as instruments, need conditional exogeneity of each share) OR exogenous shocks (Borusyak et al. 2022: identification from many independent shocks, inference at the shock level). The researcher must argue which source of exogeneity is credible.", question: "Order the steps for implementing a shift-share IV analysis.", options: ["Define exposure shares from a pre-period (e.g., immigrant settlement patterns)", "Identify aggregate shocks independent of local conditions (e.g., national-origin immigration flows)", "Construct the Bartik instrument as the share-weighted sum of shocks", "Argue identification source: exogenous shares (GPSS) or exogenous shocks (Borusyak et al.)", "Run first-stage and assess instrument strength", "Estimate 2SLS and interpret as LATE for complying regions"], answer: [0, 1, 2, 3, 4, 5], feedback: "Correct. Shift-share IV requires clearly defining shares, shocks, construction, identification argument, diagnostics, and interpretation." },
  "iv-boss": { quest: "iv", n: "Boss", title: "Housing Court Judge IV: Policy Memo", xp: 150, skills: ["iv"], badge: "iv-master", type: "memo", story: "Boss project: A legal aid organization asks you to estimate the causal effect of eviction on homelessness, earnings, and children's school outcomes. You have access to housing court records with quasi-random judge assignment. Write a policy memo explaining your IV strategy.", concept: "A credible IV memo identifies the endogenous variable, instrument, argues for the three IV assumptions, reports first-stage strength, interprets LATE, and discusses external validity limitations.", question: "Write a policy memo (200+ chars) explaining: the endogeneity problem, your judge-leniency instrument, exclusion restriction argument, first-stage F requirement, LATE interpretation (who are the compliers?), and one policy implication.", min: 200, required: ["endogen", "instrument", "exclusion", "first stage", "LATE", "complier"], feedback: "Boss cleared. You wielded an instrument — identification, diagnostics, and interpretation in one coherent argument." },

  "rdd-1": { quest: "rdd", n: 1, title: "Sharp RDD Concept", xp: 30, skills: ["rdd"], type: "mcq", story: "A scholarship program awards full tuition to students scoring ≥ 80 on a standardized exam. Students scoring 79 receive nothing. You want to estimate the causal effect of the scholarship on college completion.", concept: "In a sharp RDD, treatment switches deterministically at a known cutoff of a running variable (also called forcing variable or score). Identification comes from local randomization: units just above and below the cutoff are similar in all observed and unobserved characteristics, differing only in treatment. The key assumption is continuity of potential outcomes at the cutoff.", question: "In a sharp RDD, what determines whether a unit is treated?", options: ["Whether the running variable exceeds a known cutoff", "Random assignment by a researcher", "Self-selection into treatment", "A judge's subjective decision"], answer: 0, feedback: "Correct. Sharp RDD assigns treatment deterministically based on whether the score crosses the threshold." },
  "rdd-2": { quest: "rdd", n: 2, title: "Bandwidth Selection", xp: 35, skills: ["rdd"], type: "mcq", story: "You choose a bandwidth of ±2 points around the cutoff. Your advisor says \"too narrow, no power.\" You widen to ±20. She says \"too wide, bias from functional form.\"", concept: "Bandwidth governs the bias-variance tradeoff: narrow bandwidths reduce bias (only very similar units) but increase variance (fewer observations). Wide bandwidths increase precision but introduce bias if the conditional expectation function is misspecified away from the cutoff. Optimal bandwidth selectors (Imbens-Kalyanaraman 2012, Calonico-Cattaneo-Titiunik 2014) formalize this tradeoff by minimizing MSE. Robustness checks: show results across a range of bandwidths.", question: "What is the fundamental tradeoff in RDD bandwidth selection?", options: ["Bias (from functional form misspecification) vs. variance (from fewer observations)", "Speed vs. accuracy of code execution", "Number of fixed effects vs. degrees of freedom", "Clustering level vs. sample size"], answer: 0, feedback: "Correct. The MSE-optimal bandwidth balances approximation bias against estimation variance." },
  "rdd-3": { quest: "rdd", n: 3, title: "Local Polynomial Estimation", xp: 35, skills: ["rdd"], type: "code", story: "You implement an RDD using the rdrobust package. The key choice is the polynomial order and kernel function for the local regression on each side of the cutoff.", concept: "RDD estimation fits separate local polynomials on each side of the cutoff, weighted by a kernel (triangular is default). Linear local polynomials (p=1) are standard; higher orders reduce boundary bias but increase variance. The treatment effect is the discontinuity: the difference in intercepts at the cutoff. rdrobust (Calonico, Cattaneo, Titiunik) implements bias-corrected inference with robust confidence intervals.", question: "Complete the rdrobust call to estimate the RDD effect of the scholarship on college completion.", starter: "rdrobust(Y = df$completion, X = df$score, c = _____, p = _____)", required: ["80", "1"], pattern: "c\\s*=\\s*80.*p\\s*=\\s*1", hint: "The cutoff is 80 (the scholarship threshold) and local linear (p=1) is standard.", feedback: "Good. rdrobust with c=80 and p=1 estimates the local linear RD effect with bias-corrected CI." },
  "rdd-4": { quest: "rdd", n: 4, title: "Manipulation and Validity Checks", xp: 35, skills: ["rdd"], type: "ordering", story: "Before reporting your RDD estimate, you need to verify that the design is credible. Khanna (JPE 2023) provides an excellent example of thorough validity checks in an education RDD in India.", concept: "RDD validity requires: (1) No manipulation — agents cannot precisely sort around the cutoff. Test via McCrary (2008) density test or Cattaneo-Jansson-Ma (2020) test. (2) Covariate balance — predetermined covariates should be smooth through the cutoff (run RDD on each covariate as a placebo outcome). (3) No other discontinuities — no other programs change at the same cutoff. (4) Robustness — stable across bandwidths and polynomial orders.", question: "Order the RDD validity checks from first to last.", options: ["Run McCrary/density test for manipulation of the running variable", "Check covariate balance at the cutoff (placebo RDD on pre-treatment variables)", "Verify no other policies change at the same threshold", "Show robustness across bandwidths and polynomial orders"], answer: [0, 1, 2, 3], feedback: "Correct. Manipulation → covariate balance → institutional check → robustness is the standard validation sequence." },
  "rdd-5": { quest: "rdd", n: 5, title: "Fuzzy RDD", xp: 40, skills: ["rdd", "iv"], type: "mcq", story: "The scholarship threshold is 80, but some students above 80 decline the scholarship, and a few below 80 receive it through appeals. Treatment is not a deterministic function of the score.", concept: "Fuzzy RDD arises when the probability of treatment jumps at the cutoff but doesn't go from 0 to 1. This is equivalent to an IV setup where the instrument is \"above cutoff\" (Z = 1[X ≥ c]). The Wald estimator: τ_FRD = (jump in E[Y|X] at c) / (jump in E[D|X] at c). The numerator is the reduced-form discontinuity; the denominator is the first-stage discontinuity in treatment take-up. Like IV, fuzzy RDD estimates a LATE for compliers at the cutoff.", question: "In a fuzzy RDD, what role does the cutoff indicator play?", options: ["It serves as an instrument for actual treatment — fuzzy RDD is IV at the cutoff", "It directly estimates ATE for the full population", "It eliminates the need for bandwidth selection", "It replaces the running variable entirely"], answer: 0, feedback: "Correct. Fuzzy RDD = IV where the instrument is the cutoff indicator, identifying LATE for compliers at the threshold." },
  "rdd-6": { quest: "rdd", n: 6, title: "RDD Visualization", xp: 40, skills: ["rdd"], type: "mcq", story: "Korting et al. (QJE 2023) argue that most RD plots in published papers obscure rather than reveal. They propose \"visual inference\" — can a viewer distinguish the real discontinuity from placebo-shuffled plots?", concept: "Best practices from Korting et al. (QJE 2023): (1) Plot raw or binned data with the fitted function overlaid — don't just show the fit. (2) Avoid fitting global polynomials that exaggerate the jump. (3) Use evenly-spaced bins of the running variable (not quantile bins). (4) Show the confidence interval on the local polynomial fit. (5) The \"visual inference\" test: embed the real plot among placebo plots (permute treatment around the cutoff) and check if viewers can identify it. If they can't, the discontinuity may be too fragile.", question: "According to Korting et al. (QJE 2023), which RD plot practice is most misleading?", options: ["Fitting high-order global polynomials that mechanically create a visual gap", "Using evenly-spaced bins of the running variable", "Showing raw data points alongside the local fit", "Reporting the bandwidth used"], answer: 0, feedback: "Correct. Global polynomials are sensitive to observations far from the cutoff and can fabricate visual discontinuities." },
  "rdd-boss": { quest: "rdd", n: "Boss", title: "Scholarship Eligibility RDD", xp: 150, skills: ["rdd", "iv"], badge: "rdd-master", type: "memo", story: "Boss project: A state education board wants to know whether merit scholarships (awarded at score ≥ 80) improve 4-year graduation rates. Some students above 80 don't take up the scholarship; some below 80 receive partial aid through a different channel. Design the RDD study.", concept: "A complete RDD memo specifies: running variable, cutoff, sharp or fuzzy design, bandwidth selection procedure, polynomial order, manipulation test plan, covariate balance checks, estimation command, and interpretation of the LATE at the cutoff.", question: "Write a research design memo (200+ chars) specifying: the running variable and cutoff, whether this is sharp or fuzzy (and why), your bandwidth selection approach, validity checks (McCrary + covariate balance), estimation strategy, and what the estimate identifies (LATE at the cutoff).", min: 200, required: ["running variable", "cutoff", "fuzzy", "bandwidth", "McCrary", "covariate", "LATE"], feedback: "Boss cleared. You designed an RDD from identification to inference — the Discontinuity Detective badge is earned." },

  "match-1": { quest: "matching", n: 1, title: "Selection on Observables", xp: 30, skills: ["psm"], type: "mcq", story: "A government job training program is offered to all unemployed workers, but only some choose to participate. You observe demographics, prior earnings, education, and region for both participants and non-participants.", concept: "The Conditional Independence Assumption (CIA), also called unconfoundedness or selection on observables, states: conditional on observed covariates X, treatment assignment D is independent of potential outcomes (Y(0), Y(1)). Formally: (Y(0), Y(1)) ⊥ D | X. This means all confounders are observed — a strong and untestable assumption. If unobserved factors (e.g., motivation) also drive selection, CIA fails and you need IV or RDD instead.", question: "What does the Conditional Independence Assumption (CIA) require?", options: ["All confounders affecting both treatment and outcome are observed and controlled for", "Treatment is randomly assigned", "The running variable crosses a cutoff", "An instrument shifts treatment status"], answer: 0, feedback: "Correct. CIA assumes no unobserved confounders — selection into treatment is fully explained by observables." },
  "match-2": { quest: "matching", n: 2, title: "Propensity Score Logic", xp: 30, skills: ["psm"], type: "mcq", story: "You have 50 covariates. Matching on all 50 dimensions simultaneously is computationally infeasible — the curse of dimensionality. Rosenbaum & Rubin (1983) proposed a solution.", concept: "The propensity score e(X) = P(D=1|X) is a scalar summary of covariate information. The key theorem: if CIA holds conditional on X, it also holds conditional on e(X). This reduces a high-dimensional matching problem to a one-dimensional one. The balancing property: within strata of equal propensity scores, the distribution of X should be the same for treated and control units. Overlap/common support: e(X) must be bounded away from 0 and 1.", question: "What problem does the propensity score solve?", options: ["Curse of dimensionality — reduces multi-dimensional matching to a single score", "Weak instruments in 2SLS", "Manipulation of the running variable", "Serial correlation in panel data"], answer: 0, feedback: "Correct. The propensity score collapses high-dimensional X into a scalar balancing score." },
  "match-3": { quest: "matching", n: 3, title: "Matching Estimators", xp: 35, skills: ["psm"], type: "mcq", story: "You estimate propensity scores via logit and now must choose a matching algorithm: nearest-neighbor (with or without replacement), kernel matching, radius/caliper matching.", concept: "Nearest-neighbor matching pairs each treated unit with the control unit(s) having the closest propensity score. With replacement allows a control to be reused (reduces bias, increases variance). Kernel matching uses all controls weighted by distance in propensity score (smooth, uses more data). Caliper matching drops treated units without a close match (improves quality, may lose sample). After matching, check covariate balance (standardized differences < 0.1). Abadie-Imbens (2006) provide bias-corrected matching estimators and valid standard errors.", question: "What is the main advantage of nearest-neighbor matching WITH replacement over without replacement?", options: ["Each treated unit gets the best available match, reducing bias from poor matches", "It always increases sample size", "Standard errors become exactly zero", "It eliminates the need for the propensity score"], answer: 0, feedback: "Correct. With-replacement matching allows optimal pairing for each treated unit, at the cost of higher variance when few controls are reused heavily." },
  "match-4": { quest: "matching", n: 4, title: "Inverse Probability Weighting", xp: 35, skills: ["psm"], type: "fill", story: "Instead of matching, you weight each observation by the inverse of its probability of receiving the treatment it actually received. A treated unit with e(X) = 0.9 is common (low weight); one with e(X) = 0.2 is rare among treated (high weight).", concept: "IPW creates a pseudo-population where treatment is independent of X. For ATT: weight controls by e(X)/(1-e(X)). For ATE: weight treated by 1/e(X) and controls by 1/(1-e(X)). Extreme weights arise when propensity scores are near 0 or 1 (poor overlap) — trimming or normalized/stabilized weights help. IPW is consistent if the propensity score model is correctly specified, but can be high-variance with extreme weights.", question: "For the ATE, what is the IPW weight for a treated unit (D=1) with propensity score e(X) = 0.4?", answer: ["2.5", "1/0.4"], feedback: "Correct. For treated units under ATE-IPW, weight = 1/e(X) = 1/0.4 = 2.5." },
  "match-5": { quest: "matching", n: 5, title: "Doubly Robust / AIPW", xp: 40, skills: ["psm", "dml"], type: "mcq", story: "A co-author worries: \"What if our propensity score model is wrong?\" Another worries: \"What if our outcome regression is wrong?\" AIPW addresses both concerns simultaneously.", concept: "Augmented Inverse Probability Weighting (AIPW) combines an outcome model μ(X) with a propensity model e(X). The estimator is doubly robust: it is consistent if EITHER the outcome model OR the propensity model is correctly specified (only one needs to be right). The formula augments IPW with a bias-correction term from the outcome model. AIPW is also locally semiparametrically efficient — it achieves the lowest possible variance among regular estimators when both models are correct (Robins, Rotnitzky & Zhao 1994).", question: "Why is the AIPW estimator called \"doubly robust\"?", options: ["It is consistent if either the propensity score or the outcome model is correctly specified", "It uses two instruments instead of one", "It requires two running variables", "It clusters at two levels simultaneously"], answer: 0, feedback: "Correct. Double robustness gives you two chances to get the specification right — a major practical advantage." },
  "match-6": { quest: "matching", n: 6, title: "Double/Debiased Machine Learning", xp: 40, skills: ["dml"], type: "mcq", story: "You have hundreds of potential confounders and want to use ML (random forests, LASSO) to model them flexibly. But plugging ML predictions naively into a treatment effect estimator introduces regularization bias. Chernozhukov et al. (Econometrica 2018) show how to fix this.", concept: "Double/Debiased Machine Learning (DML) solves two problems: (1) Regularization bias — ML models shrink predictions, biasing the treatment effect estimate. Fixed by Neyman orthogonality: construct a moment condition whose derivative with respect to nuisance parameters is zero, so small errors in nuisance estimation don't propagate. (2) Overfitting bias — using the same sample to fit nuisance models and estimate the effect causes bias. Fixed by cross-fitting (sample splitting): fit the ML model on a fold, predict on the held-out fold, and estimate the causal parameter on the predictions. DML achieves √n-consistent, asymptotically normal inference on the causal parameter even when nuisance functions are estimated at slower-than-√n rates.", question: "What is the role of cross-fitting in Double ML?", options: ["Prevents overfitting bias by ensuring nuisance predictions are out-of-sample", "Replaces the need for any outcome model", "Eliminates the exclusion restriction requirement", "Selects the optimal bandwidth for RDD"], answer: 0, feedback: "Correct. Cross-fitting ensures ML-estimated nuisance parameters don't overfit to the estimation sample, preserving valid inference." },
  "match-boss": { quest: "matching", n: "Boss", title: "Job Training Program Evaluation", xp: 150, skills: ["psm", "dml"], badge: "matching-master", type: "memo", story: "Boss project: A workforce development agency asks you to evaluate a job training program using observational data. You observe rich baseline covariates (demographics, prior earnings history, education, region) but cannot run an experiment. Write a memo proposing and defending your estimation strategy.", concept: "A strong treatment-effect memo under selection on observables should: state CIA and why it might hold here, choose between matching/IPW/AIPW/DML with justification, describe covariate balance diagnostics, address overlap concerns, and interpret the estimand (ATT vs ATE).", question: "Write a policy memo (200+ chars) explaining: why CIA might hold (what confounders you observe), your estimation strategy (pick AIPW or DML and justify), how you check balance, how you handle overlap violations, and what estimand (ATT/ATE) you target and why.", min: 200, required: ["CIA", "propensity", "balance", "overlap", "doubly robust", "ATT"], feedback: "Boss cleared. You constructed counterfactuals from observational data — the Counterfactual Constructor badge is earned." },

  "ts-1": { quest: "ts", n: 1, title: "Time Series Fundamentals", xp: 30, skills: ["ts"], type: "mcq", story: "You receive monthly GDP growth, unemployment, and the federal funds rate. Before estimating dynamic effects, you need to assess basic time series properties.", concept: "Key properties: (1) Stationarity — a process is (weakly) stationary if its mean, variance, and autocovariances do not depend on time. Non-stationary series (unit roots, trends) require differencing or detrending. (2) Autocorrelation — observations are correlated with their own lags; this affects inference and motivates dynamic models. (3) Trends — deterministic (time polynomials) vs. stochastic (unit root/random walk). ADF or KPSS tests assess stationarity. Ignoring non-stationarity risks spurious regression.", question: "What does weak stationarity require of a time series?", options: ["Constant mean, constant variance, and autocovariances that depend only on lag distance", "Observations must be independent over time", "The series must be normally distributed", "All values must be positive"], answer: 0, feedback: "Correct. Weak stationarity means the first two moments and the autocovariance function are time-invariant." },
  "ts-2": { quest: "ts", n: 2, title: "VAR and Impulse Response Functions", xp: 35, skills: ["ts"], type: "mcq", story: "You estimate a 3-variable VAR(4) with GDP growth, inflation, and the interest rate. Your advisor asks for impulse response functions showing how a monetary policy shock propagates through the system.", concept: "A Vector Autoregression (VAR) models each variable as a linear function of its own lags and lags of all other variables in the system. Impulse Response Functions (IRFs) trace how a one-unit shock to one variable propagates to all variables over time. Identification of structural shocks requires restrictions: Cholesky ordering (lower triangular) assumes a recursive causal ordering — variables ordered earlier can affect later ones contemporaneously but not vice versa. The ordering matters and must be economically motivated.", question: "In Cholesky-identified VARs, what does the ordering of variables determine?", options: ["Which variables can affect others contemporaneously — earlier variables are causally prior within the period", "The number of lags in the VAR", "The stationarity of each series", "The bandwidth for kernel estimation"], answer: 0, feedback: "Correct. Cholesky ordering imposes a recursive contemporaneous causal structure — a strong and often controversial assumption." },
  "ts-3": { quest: "ts", n: 3, title: "Local Projections", xp: 35, skills: ["lp"], type: "code", story: "Jordà (AER 2005) proposes an alternative to VARs: instead of iterating a one-step-ahead model, directly regress the outcome at each horizon h on the current shock. Jordà & Taylor (JEL 2025) provide a comprehensive review.", concept: "Local Projections estimate impulse responses via a sequence of direct regressions: y_{t+h} = α_h + β_h · shock_t + γ_h · controls_t + ε_{t+h}, for h = 0, 1, 2, ..., H. Each horizon h gives one point on the IRF. No iteration, no inversion of a companion matrix. Advantages: robust to misspecification of the VAR lag structure, easy to add non-linearities, and naturally handles the moving-average structure in residuals (though inference requires Newey-West or clustering).", question: "Complete the local projection regression for horizon h=4 (outcome 4 periods ahead on the shock today).", starter: "feols(lead(y, _____) ~ shock + lag(y, 1) + lag(y, 2), data = df, vcov = \"NW\" ~ 4)", required: ["4"], pattern: "lead\\(y,\\s*4\\)", hint: "The dependent variable is y at t+h, so lead(y, 4) for horizon 4.", feedback: "Good. Each LP regression estimates one point on the impulse response, with Newey-West SEs correcting for serial correlation." },
  "ts-4": { quest: "ts", n: 4, title: "LP vs VAR", xp: 35, skills: ["lp", "ts"], type: "mcq", story: "Your co-author prefers VARs (\"more efficient!\"). You prefer LPs (\"more robust!\"). Jordà & Taylor (JEL 2025) lay out the tradeoffs clearly.", concept: "VAR advantages: more efficient when the true DGP is a finite-order VAR (all horizons estimated jointly from one model); tighter confidence bands. LP advantages: (1) robust to misspecification — doesn't compound errors across horizons; (2) easily incorporates state dependence, non-linearities, and external instruments; (3) doesn't require specifying the full system. LP disadvantages: less efficient (wider CIs), requires care with serial correlation in residuals (Newey-West/HAC or clustering). In practice, many applied researchers prefer LP for robustness and show VAR for efficiency comparison.", question: "What is the main efficiency advantage of VARs over local projections?", options: ["VARs use all horizons jointly from one iterated model, giving tighter confidence bands when correctly specified", "VARs don't require any identifying assumptions", "VARs are always unbiased regardless of lag length", "VARs automatically correct for non-stationarity"], answer: 0, feedback: "Correct. When the VAR is correctly specified, iterated IRFs are more efficient. But misspecification compounds across horizons — LP's robustness often wins in practice." },
  "ts-5": { quest: "ts", n: 5, title: "LP-IV: Causal Impulse Responses", xp: 40, skills: ["lp", "iv"], type: "mcq", story: "You want to estimate the causal effect of monetary policy shocks on output over time. But the interest rate is endogenous (the Fed responds to economic conditions). You need an external instrument — e.g., Romer & Romer narrative shocks or high-frequency identification around FOMC announcements.", concept: "LP-IV combines local projections with instrumental variables: y_{t+h} = α_h + β_h · X_t + controls + ε_{t+h}, where X (e.g., interest rate change) is instrumented by Z (e.g., narrative monetary shock). This gives a causal IRF — each β_h is the causal effect at horizon h, identified by the external instrument. Requirements: Z must be relevant (predict X), exogenous (uncorrelated with confounders), and satisfy the exclusion restriction (affects y_{t+h} only through X_t). Stock & Watson (2018) and Jordà & Taylor (JEL 2025) formalize this.", question: "What does LP-IV achieve that standard LP cannot?", options: ["Causal identification of impulse responses using an external instrument for the endogenous shock", "Wider confidence intervals for more conservative inference", "Automatic stationarity of the outcome variable", "Elimination of all serial correlation in residuals"], answer: 0, feedback: "Correct. LP-IV identifies causal dynamic effects by instrumenting the endogenous variable at each horizon." },
  "ts-6": { quest: "ts", n: 6, title: "LP-DiD: Bridging Micro and Macro", xp: 40, skills: ["lp", "did"], type: "mcq", story: "Dube, Girardi, Jordà & Taylor develop LP-DiD — combining the local projection framework with difference-in-differences for panel settings with staggered treatment. This bridges microeconometric causal inference with macroeconometric dynamics.", concept: "LP-DiD estimates dynamic treatment effects in panel data by running local projections of the form: y_{i,t+h} - y_{i,t-1} = α_h + β_h · D_{it} + controls + ε_{i,t+h}. The differencing (or inclusion of unit FE) handles time-invariant confounders; the LP framework traces effects over time without imposing a parametric lag structure. Advantages over event-study with leads/lags: (1) cleaner separation of horizons; (2) avoids issues with relative-time dummies in staggered settings; (3) naturally handles heterogeneous treatment timing. Clean counterfactual: only not-yet-treated units serve as controls at each horizon.", question: "How does LP-DiD improve upon standard event-study specifications in staggered settings?", options: ["It uses not-yet-treated units as clean controls at each horizon, avoiding contamination from already-treated units", "It eliminates the need for parallel trends assumptions", "It requires no panel data structure", "It always produces narrower confidence intervals than TWFE"], answer: 0, feedback: "Correct. LP-DiD naturally handles staggered timing by using only clean controls at each projection horizon, avoiding the \"bad comparison\" problem of TWFE event studies." },
  "ts-boss": { quest: "ts", n: "Boss", title: "Macro Policy Shock: Impulse Responses", xp: 150, skills: ["ts", "lp", "iv"], badge: "lp-master", type: "memo", story: "Boss project: A central bank research department asks you to estimate how an unexpected 25bp interest rate hike propagates to GDP growth, unemployment, and inflation over 24 months. You have a narrative shock series (à la Romer & Romer) as an external instrument. Write a research design memo.", concept: "A complete LP-IV memo should specify: the endogenous shock, the external instrument and why it satisfies IV assumptions, the LP specification (horizons, controls, lag structure), inference method (Newey-West or cluster), comparison with VAR-based IRFs, and interpretation of the dynamic causal effects.", question: "Write a research design memo (200+ chars) explaining: the endogeneity problem (why raw interest rate changes aren't causal), your external instrument (narrative shock) and its validity, the LP-IV specification for horizons 0-24, how you handle inference with serially correlated residuals, and how you'd compare results with a Cholesky VAR.", min: 200, required: ["endogen", "instrument", "local projection", "horizon", "Newey-West", "VAR"], feedback: "Boss cleared. You traced impulse responses with causal identification — the Impulse Tracer badge is earned." }
};
...
function isQuestUnlocked(q) {
  ...
  if (q.id === "iv") return isDone("did-boss");
  if (q.id === "rdd") return isDone("iv-boss");
  if (q.id === "matching") return isDone("did-boss");
  if (q.id === "ts") return isDone("fe-boss");
  return false;
}
...
function questPrerequisiteText(q) {
  ...
  if (q.id === "iv") return t('prereq_iv');
  if (q.id === "rdd") return t('prereq_rdd');
  if (q.id === "matching") return t('prereq_matching');
  if (q.id === "ts") return t('prereq_ts');
  return "";
}
...
function questPrerequisite(q) {
  ...
  if (q.id === "iv") return t('prereq_iv_full');
  if (q.id === "rdd") return t('prereq_rdd_full');
  if (q.id === "matching") return t('prereq_matching_full');
  if (q.id === "ts") return t('prereq_ts_full');
  return "";
}
...
function isQuestUnlockedWith(doneSet, q) {
  ...
  if (q.id === "iv") return doneSet.has("did-boss");
  if (q.id === "rdd") return doneSet.has("iv-boss");
  if (q.id === "matching") return doneSet.has("did-boss");
  if (q.id === "ts") return doneSet.has("fe-boss");
  return false;
}
